...
bool ThreadController::add(Thread* _thread) {
  ...
  for (int i = 0; i < MAX_THREADS; i++) {
    if (thread[i] != NULL && thread[i]->ThreadID == _thread->ThreadID) return true;
  }
...
void ThreadController::remove(Thread* _thread) { remove(_thread->ThreadID); }
